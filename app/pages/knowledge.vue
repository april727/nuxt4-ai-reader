<template>
  <div class="kp-page">
    ...
    <aside class="kp-sidebar">
      ...
      <div class="kp-groups-list">
        ...
        <div v-if="sourceGroups.length" class="kp-groups-section">
          <div class="kp-groups-label">来源资料</div>
          <button
        ...
        <!-- 自定义分组 -->
        <div v-if="customGroups.length || true" class="kp-groups-section">
    ...
    <!-- 中间标题列表 -->
    <div class="kp-list" v-if="activeGroup">
    ...
    <!-- 右侧内容 + AI 对话 -->
    <main class="kp-detail" v-if="selectedItem">
      <div class="kp-detail-header">
        ...
        <button class="kp-detail-del" @click="deleteItem(selectedItem.id)">删除</button>
      </div>
...
function getGroupLabel(key: string, item: KpItem) { return item.customGroup || item.sourceTitle || '未分类' }
...
const activeItems = computed(() => {
  return items.value.filter(i => getGroupKey(i) === activeGroup.value)
})

const selectedItem = computed(() => items.value.find(i => i.id === selectedId.value) || null)

// ── 分组操作 ──
function selectGroup(key: string) { activeGroup.value = key; selectedId.value = null; nextTick(() => { const f = activeItems.value[0]; if (f) selectedId.value = f.id }) }
...
function typeLabel(t: string) { return { note: '笔记', podcast_note: '播客', ai_qa: 'AI', selection: '选中', manual: '手动' }[t] || t }
...
async function deleteItem(id: string) { try { await $fetch(`/api/knowledge/${id}`, { method: 'DELETE' }); items.value = items.value.filter(i => i.id !== id) } catch {} }

onMounted(() => { loadItems() })
// 点击空白关闭右键菜单
onMounted(() => document.addEventListener('click', () => { groupMenu.key = '' }))
</script>
...
.kp-list-title { font-size: 13px; font-weight: 600; color: #1a1a18; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.kp-list-item { padding: 10px 12px; border-radius: 8px; cursor: grab; transition: background 0.12s; border: 0.5px solid transparent; }
.kp-list-item:active { cursor: grabbing; }
...
.kp-detail-header { display: flex; align-items: center; gap: 8px; padding: 14px 20px 10px; border-bottom: 0.5px solid rgba(0,0,0,0.05); flex-shrink: 0; background: #fff; }
...
.kp-detail-body { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; }
.kp-detail-content { font-size: 14px; line-height: 1.8; color: #1a1a18; font-family: 'Lora', Georgia, serif; white-space: pre-wrap; padding-bottom: 24px; border-bottom: 0.5px solid rgba(0,0,0,0.06); }
.kp-detail-content :deep(p) { margin: 0.5em 0; }

.kp-chat { margin-top: 20px; flex: 1; display: flex; flex-direction: column; min-height: 0; }
